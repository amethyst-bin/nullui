local NullLibrary = {
    Theme = {
        -- Цвета теперь имеют прозрачность (Glass Effect)
...
-- Умный загрузчик картинок по URL (превращает прямые ссылки в ассеты Roblox)
...
function NullLibrary:Notify(options)
    ...
    local card = create("CanvasGroup", {
        ...
        BackgroundTransparency = 0.2, -- Стекло в уведомлениях
...
function NullLibrary:CreateWindow(options)
    ...
    corner(root, 8)
    stroke(root, 0.3, 1, self.Theme.Text) -- Тонкая стеклянная обводка
    ...
    local subtitle = autosizeText(create("TextLabel", {
        ...
        Font = Enum.Font.GothamMedium,
        Size = UDim2.new(1, 0, 0, 0),
        Text = options.Subtitle or "glassmorphism ui",
        TextColor3 = self.Theme.Muted,
        TextSize = 12,
        TextXAlignment = Enum.TextXAlignment.Left,
        Parent = titleWrap
    ...
    -- Вкладки Top/Bottom перенесены в ROOT, чтобы не отставали и не улетали!
    ...
    corner(floatingTabsBar, 8)
    stroke(floatingTabsBar, 0.6, 1)
    padding(floatingTabsBar, 6, 6)
    ...
    function window:_syncFloatingTabs()
        ...
        local barWidth = math.max(200, (tabsCount * 130) + 20) -- Динамическая ширина под табы
        
        -- Плавающие табы теперь внутри Root, используем AnchorPoint
        self.FloatingTabs.Size = UDim2.fromOffset(barWidth, 48)
        self.FloatingTabs.Position = self.TabPosition == "Top" 
            and UDim2.new(0.5, 0, 0, -60) -- Выше UI
            or UDim2.new(0.5, 0, 1, 60)  -- Ниже UI
    ...
    topbar.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            isDragging = true
            dragOffset = Vector2.new(root.AbsolutePosition.X - input.Position.X, root.AbsolutePosition.Y - input.Position.Y) + (root.AbsoluteSize / 2)
            
            local dragConn, dropConn
            dragConn = UIS.InputChanged:Connect(function(input2)
                if input2.UserInputType == Enum.UserInputType.MouseMovement or input2.UserInputType == Enum.UserInputType.Touch then
                    targetPos = UDim2.new(0, input2.Position.X + dragOffset.X, 0, input2.Position.Y + dragOffset.Y)
                end
            end)
    ...
    -- RenderStepped для идеальной плавности Drag/Resize (как в macOS)
    RunService.RenderStepped:Connect(function(dt)
        if isDragging then
            root.Position = root.Position:Lerp(targetPos, math.clamp(dt * 18, 0, 1))
        end
        if isResizing then
            window.CurrentSize = window.CurrentSize:Lerp(targetSize, math.clamp(dt * 18, 0, 1))
...
function Tab:CreateSection(sectionOptions, maybeDescription)
    ...
    function section:AddImage(options)
        ...
        -- Если картинка скачивается, то url подгрузится чуть позже, чтобы UI не зависал, мы загружаем асинхронно
